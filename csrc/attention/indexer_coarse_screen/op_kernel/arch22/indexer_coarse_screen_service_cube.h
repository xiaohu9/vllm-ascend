 ...
 */
#ifndef INDEXER_COARSE_SCREEN_SERVICE_CUBE_H
#define INDEXER_COARSE_SCREEN_SERVICE_CUBE_H
...
class IndexerCoarseScreenServiceCube {
public:
    using Q_T = typename LIT::queryType;
    using K_T = typename LIT::keyType;

    __aicore__ inline IndexerCoarseScreenServiceCube(){};
    ...
    __aicore__ inline void ComputeMm1(const IndexerCoarseScreenCommon::RunInfo &runInfo);
...
protected:
    __aicore__ inline void Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize,
                                uint64_t s2L0RealSize, const IndexerCoarseScreenCommon::RunInfo &runInfo);
    __aicore__ inline void ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const IndexerCoarseScreenCommon::RunInfo &runInfo);
    ...
    __aicore__ inline void LoadQueryToL0a(uint64_t s1gL1Offset, uint64_t s1gL0Offset, uint64_t s1gL1RealSize,
                                          uint64_t s1gL0RealSize, const IndexerCoarseScreenCommon::RunInfo &runInfo);
    __aicore__ inline void QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gL1Offset, const IndexerCoarseScreenCommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset, const IndexerCoarseScreenCommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset, const IndexerCoarseScreenCommon::RunInfo &runInfo);
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::InitParams(const ConstInfo &constInfo)
{
    constInfo_ = constInfo;
}

template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::InitBuffers(TPipe *pipe)
{
...
IndexerCoarseScreenServiceCube<LIT>::InitMm1GlobalTensor(const GlobalTensor<int32_t> &blkTableGm,
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::ComputeMm1(const IndexerCoarseScreenCommon::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        if (PAGE_ATTENTION) {
            // coarse_screen:主 pass 扫全前缀,连续逻辑位置经块表直读 PA cache(生产 lightning_indexer 原生语义)
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset,
                                                    const IndexerCoarseScreenCommon::RunInfo &runInfo)
{
...
// 连续逻辑位置经块表直读 PA cache(生产 lightning_indexer KeyNd2NzForPA 原生语义):
// slot(p) = blockTable[bIdx, p/blkSize]*blkSize + p%blkSize,按块粒度连续搬运
template <typename LIT>
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::LoadQueryToL0a(uint64_t s1gGmOffset,
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::LoadKeyToL0b(uint64_t s2L1Offset,
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize,
                                                const IndexerCoarseScreenCommon::RunInfo &runInfo)
{
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::Fixp(uint64_t s1gGmOffset,
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::AllocEventID()
{
...
template <typename LIT>
__aicore__ inline void IndexerCoarseScreenServiceCube<LIT>::FreeEventID()
{
