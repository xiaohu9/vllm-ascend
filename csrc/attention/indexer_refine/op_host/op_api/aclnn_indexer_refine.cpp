...
#include "aclnn_indexer_refine.h"
...
extern aclnnStatus aclnnInnerIndexerRefineGetWorkspaceSize(
    const aclTensor *query, const aclTensor *key, const aclTensor *weights, const aclTensor *candidates,
...
extern aclnnStatus aclnnInnerIndexerRefine(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
...
aclnnStatus aclnnIndexerRefineGetWorkspaceSize(
        const aclTensor *query,
        const aclTensor *key,
        const aclTensor *weights,
        const aclTensor *candidates,
        ...
        aclOpExecutor **executor)
{
    ...
    return aclnnInnerIndexerRefineGetWorkspaceSize(
        query, key, weights, candidates, actualSeqLengthsQueryOptional, actualSeqLengthsKeyOptional,
        blockTableOptional, layoutQueryOptional, layoutKeyOptional, sparseCount, sparseIndicesOut, workspaceSize,
        executor);
}

aclnnStatus aclnnIndexerRefine(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                     const aclrtStream stream)
{
    return aclnnInnerIndexerRefine(workspace, workspaceSize, executor, stream);
}
