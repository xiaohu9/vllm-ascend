...
/*!
 * \file indexer_refine_kernel.h
 * \brief
 */

#ifndef INDEXER_REFINE_KERNEL_H
#define INDEXER_REFINE_KERNEL_H
...
#include "indexer_refine_service_vector.h"
#include "indexer_refine_service_cube.h"
...
class IndexerRefineKernel {
public:
    __aicore__ inline IndexerRefineKernel(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                __gm__ uint8_t *candidates, __gm__ uint8_t *actualSeqLengthsQ,
                                __gm__ uint8_t *actualSeqLengths,
                                __gm__ uint8_t *blockTable, __gm__ uint8_t *sparseIndices, __gm__ uint8_t *workspace,
                                const IndexerRefineTilingData *__restrict tiling, TPipe *tPipe);
    ...
    IndexerRefineServiceCube<LIT> matmulService;
...
protected:
    ...
    IndexerRefineCommon::ConstInfo constInfo{};
    TempLoopInfo tempLoopInfo{};
    IndexerRefineCommon::SplitCoreInfo splitCoreInfo{};

    // ================================Init functions==================================
    __aicore__ inline void InitTilingData(const IndexerRefineTilingData *__restrict tilingData);
    ...
    __aicore__ inline void CalcRunInfo(uint32_t loop, uint32_t s2LoopIdx, IndexerRefineCommon::RunInfo &runInfo);
    __aicore__ inline void DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx, uint32_t s1Start);
};

template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::InitTilingData(const IndexerRefineTilingData *__restrict tilingData)
{
    ...
    constInfo.mBaseSizeAlign = IndexerRefineCommon::Align(constInfo.mBaseSize, BLOCK_CUBE_SIZE);
}

template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
                                                        __gm__ uint8_t *actualSeqLengths)
{
...
template <typename LIT>
__aicore__ inline uint32_t IndexerRefineKernel<LIT>::GetActualSeqLen(uint32_t bIdx,
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::GetS1S2ActualSeqLen(uint32_t bIdx,
...
// 多核版本，双闭区间
template <typename LIT>
__aicore__ void inline IndexerRefineKernel<LIT>::SplitCore(uint32_t curCoreIdx,
                                                         uint32_t &coreNum, IndexerRefineCommon::SplitCoreInfo &info)
{
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::Init(__gm__ uint8_t *query,
                                            __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                            __gm__ uint8_t *candidates,
                                            __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                            __gm__ uint8_t *blockTable, __gm__ uint8_t *sparseIndices,
                                            __gm__ uint8_t *workspace, const IndexerRefineTilingData *__restrict tiling,
                                            TPipe *tPipe)
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::GetBN2Idx(uint32_t bN2Idx)
{
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::CalcS2LoopParams(uint32_t bN2LoopIdx, uint32_t gS1LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::CalcGS1LoopParams(uint32_t bN2LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::CalcRunInfo(uint32_t loop,
                                                         uint32_t s2LoopIdx, IndexerRefineCommon::RunInfo &runInfo)
{
    runInfo.loop = loop;
    ...
    runInfo.actualSingleProcessSInnerSizeAlign =
        IndexerRefineCommon::Align((uint32_t)runInfo.actualSingleProcessSInnerSize, IndexerRefineCommon::ConstInfo::BUFFER_SIZE_BYTE_32B);
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::Process()
{
    if (usedCoreNum == 0) {
        // 没有计算任务，直接清理输出
        ProcessInvalid();
...
__aicore__ inline void IndexerRefineKernel<LIT>::ProcessInvalid()
{
    if ASCEND_IS_AIV {
        ...
        uint64_t singleCoreSize =
            IndexerRefineCommon::Align((totalOutputSize + aivCoreNum - 1) / aivCoreNum, GM_ALIGN_BYTES / sizeof(OUT_T));
...
__aicore__ inline void IndexerRefineKernel<LIT>::ProcessMain()
{
    ...
    IndexerRefineCommon::RunInfo runInfo;
    uint32_t gloop = 0;
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::ProcessBaseBlock(uint32_t loop,
                                                         uint64_t s2LoopIdx, IndexerRefineCommon::RunInfo &runInfo)
{
    CalcRunInfo(loop, s2LoopIdx, runInfo);
    if ASCEND_IS_AIC {
        CrossCoreWaitFlag(constInfo.syncV1C1);
        matmulService.ComputeMm1(runInfo);
        CrossCoreSetFlag<IndexerRefineCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_FIX>(constInfo.syncC1V1);
    } else {
...
template <typename LIT>
__aicore__ inline void IndexerRefineKernel<LIT>::ProcessDecode()
{
    if ASCEND_IS_AIV {
